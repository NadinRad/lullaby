document.addEventListener('DOMContentLoaded', () => {
	...
	const printFullPrice = () => {
		fullPrice.textContent = `${normalPrice(price)} Br`;
	};

	const generateCartProduct = (img, title, input, price, id) => {
		return `
			<li class="cart-content__item">
				<article class="cart-content__product cart-product" data-id="${id}">
					...
					<div class="cart-product__text">
						...
						<span class="cart-product__input">${input}</span>
						<span>x</span>
	...
	const deleteProducts = (productParent) => {
		...
		let currentCount = parseInt(productParent.querySelector('.cart-product__input').innerText);
		currentPrice = currentPrice * currentCount;
	...
	productsBtn.forEach(el => {
		...
		el.addEventListener('click', (e) => {
			...
			let input = parent.querySelector('.product-stepper__input').value;
			...
			priceNumber = priceNumber * parseInt(input);
			...
			cartProductsList.querySelector('.simplebar-content').insertAdjacentHTML('afterbegin', generateCartProduct(img, title, input, priceString, id));
			printQuantity();
	...
	const generateModalProduct = (img, title, input, price, id) => {
		return `
			<li class="order-modal__item">
				<article class="order-modal__product order-product" data-id="${id}">
					...
					<div class="order-product__text">
						...
						<span class="order-product__input">${input}</span>
						<span>x</span>
	...
	const modal = new GraphModal({
		isOpen: (modal) => {
			...
			for (item of array) {
				...
				let input = item.querySelector('.cart-product__input').innerText;
				...
				orderModalList.insertAdjacentHTML('afterbegin', generateModalProduct(img, title, input, priceString, id));
